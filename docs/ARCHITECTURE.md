...
The initial migration creates facility-scoped identity, patient, encounter, queue, clinical, ledger, cashier, and audit foundations. RLS is deny-by-default. The initial select policies demonstrate facility scoping, while direct browser mutations remain deliberately unavailable until each vertical slice has privilege-aware functions and tests.
