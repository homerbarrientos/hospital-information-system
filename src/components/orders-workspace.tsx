...
type Encounter = { id: string; encounter_no: string; patient_id: string; status: string; service_date: string; encounter_type: string };
type Order = { id: string; encounter_id: string; order_no: string; order_type: string; priority: string; status: string; ordered_at: string; instructions: string | null; version: number; cancellation_reason: string | null };
...
export function OrdersWorkspace({ patients, encounters, orders, items, results }: {
  patients: Patient[]; encounters: Encounter[]; orders: Order[]; items: Item[]; results: Result[];
}) {
  ...
  return <>
    ...
    <section className="card">
      ...
      <div className="table-wrap"><table className="data-table orders-table">
        <thead><tr><th>Order</th><th>Patient / Encounter</th><th>Requested services</th><th>Priority</th><th>Status</th><th>Action</th></tr></thead>
        <tbody>
          {orders.map((order) => {
            const encounter = encounters.find((item) => item.id === order.encounter_id);
          ...
          {!orders.length && <tr><td colSpan={6} className="empty-state">No clinical orders recorded yet.</td></tr>}
        </tbody>
      </table></div>
    </section>
    {creating && <OrderFormDialog mode="create" encounters={encounters} patients={patients} close={() => setCreating(false)}/>}
    {selected && <OrderDetailDialog order={selected} encounter={encounters.find((item) => item.id === selected.encounter_id)} patient={patientFor(selected)} items={items.filter((item) => item.order_id === selected.id)} results={results} close={() => setSelected(null)}/>}
  </>;
}
...
function OrderFormDialog({ mode, encounters, patients, order, savedItems, close }: {
  mode: "create" | "edit"; encounters: Encounter[]; patients: Patient[]; order?: Order; savedItems?: Item[]; close: () => void;
}) {
...
function OrderDetailDialog({ order, encounter, patient, items, results, close }: {
  order: Order; encounter?: Encounter; patient?: Patient; items: Item[]; results: Result[]; close: () => void;
}) {
  ...
  if (editing) return <OrderFormDialog mode="edit" encounters={encounter ? [encounter] : []} patients={patient ? [patient] : []} order={order} savedItems={items} close={() => setEditing(false)}/>;
