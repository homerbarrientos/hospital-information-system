  ...
  return <div className="modal-backdrop pharmacy-backdrop"><section className="modal pharmacy-modal" role="dialog" aria-modal="true"><ModalHead title={mode==="create"?"Add prescription":`Modify ${prescription?.prescription_no}`} close={close}/>
    <form action={action} className="pharmacy-form">
      ...
      <fieldset className="pharmacy-item-builder wide"><legend>Prescribed medicines</legend>{draftItems.map((item,index)=><div className="pharmacy-item-card" key={index}>
        ...
        <label className="medicine-dose">Dose<input required value={item.dose} onChange={event=>update(index,"dose",event.target.value)} placeholder="Example: 500 mg"/></label>
        <label className="medicine-route">Route<select required value={item.route} onChange={event=>update(index,"route",event.target.value)}>{refOptions(references,"medication_route").map(option=><option key={option.code} value={option.code}>{option.label}</option>)}</select></label>
        <label className="medicine-frequency">Frequency<select required value={item.frequency} onChange={event=>update(index,"frequency",event.target.value)}>{refOptions(references,"medication_frequency").map(option=><option key={option.code} value={option.code}>{option.label}</option>)}</select></label>
        <label className="medicine-duration">Duration<input value={item.duration} onChange={event=>update(index,"duration",event.target.value)} placeholder="Example: 7 days"/></label>
        <label className="medicine-quantity">Quantity<input required min="0.001" step="0.001" type="number" value={item.quantity} onChange={event=>update(index,"quantity",event.target.value)}/></label>
        ...
        <button type="button" className="icon-btn medicine-remove" disabled={draftItems.length===1} onClick={()=>setDraftItems(current=>current.filter((_,itemIndex)=>itemIndex!==index))} aria-label="Remove medicine"><Trash2 size={15}/></button>
