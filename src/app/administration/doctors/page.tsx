...
export default async function DoctorsPage() {
  ...
  const [{ data: assignments }, { data: departments }, {data:specialties}] = await Promise.all([
    ...
    supabase.from("reference_options").select("code,label,reference_groups!inner(code)").eq("reference_groups.code","doctor_specialty").eq("active",true).order("sort_order"),
  ]);
  ...
  return <>
    ...
    <DoctorsWorkspace doctors={doctors} departments={departments || []} facilityId={facilityId} specialties={specialties || []}/>
  </>;
}
