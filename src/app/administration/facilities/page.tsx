...
export default async function FacilitiesPage(){
 ...
 const{data:facility,error:facilityError}=await supabase.from("facilities").select("id,organization_id,code,name,level,timezone,address,phone,email,license_no,version").eq("id",facilityId).single();
 const[{data:departments,error:departmentError},{data:wards,error:wardError},{data:beds,error:bedError},{data:buildings},{data:services}]=await Promise.all([
  ...
  supabase.from("service_catalog").select("id,code,name").eq("organization_id",facility?.organization_id||"").eq("status","active").order("name")
 ]);
 const floors=(buildings||[]).flatMap(building=>(building.floors||[]).map(floor=>({id:floor.id,name:`${building.name} · ${floor.name}`})));
 const error=facilityError||departmentError||wardError||bedError;
 ...
 return <><PageHeading eyebrow="Facility governance" title="Facility and departments" description="Maintain hospital identity, departments, locations, wards, and ADT-safe bed availability."/>{error?<div className="form-error">Unable to load facility administration: {error.message}</div>:null}{facility?<FacilityWorkspace facility={facility} departments={departments||[]} wards={wards||[]} beds={facilityBeds} floors={floors} services={services||[]}/>:null}</>;
}
