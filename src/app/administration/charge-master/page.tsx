...
export default async function ChargeMasterPage(){
 ...
 const[{data:charges},{data:prices},{data:fees},{data:doctors},{data:departments},{data:categories},{data:adjustments}]=await Promise.all([
  ...
  supabase.from("doctor_fee_schedules").select("id,doctor_id,service_id,encounter_type,room_type,standard_amount,minimum_amount,maximum_amount,hospital_share_percent,effective_from,active,version").eq("facility_id",facilityId).eq("active",true).order("effective_from",{ascending:false}).limit(500),
