...
export default async function ReportsPage({searchParams}:{searchParams:Promise<{from?:string;to?:string;view?:string}>}){
 const params=await searchParams,view=["executive","operations","disease","revenue"].includes(params.view||"")?params.view||"":"",from=isoDate.test(params.from||"")?String(params.from):daysAgo(29),to=isoDate.test(params.to||"")?String(params.to):philippineDateKey(),start=periodStart(from),end=periodEnd(to);
 ...
 const title=({executive:"Executive Dashboard",operations:"Operational Analytics",disease:"Clinical Analytics",revenue:"Financial Analytics"} as Record<string,string>)[view]||"Reports";
 return <><PageHeading eyebrow="Operational intelligence" title={title} description="Facility-scoped clinical, operational, financial, inventory, and compliance reporting."/>{loadError?<div className="form-error">Unable to load reports: {loadError.message}</div>:null}<ReportsWorkspace key={view} view={view} from={from} to={to} operations={operations||[]} revenue={revenue||[]} balances={balances||[]} beds={beds||[]} diseases={diseases||[]} inventory={inventory||[]} audit={audit||[]}/></>;
}
