...
export default async function ClinicalRegistryPage(){
 ...
 const[{data:catalog},{data:encounters},{data:doctorRows},{data:fees},{data:census}]=await Promise.all([
 ...
 const[{data:diagnoses},{data:assignments},{data:patients},{data:vitals}]=await Promise.all([
  ...
  encounterIds.length?supabase.from("encounter_care_team").select("id,encounter_id,doctor_id,role,is_primary,assigned_from,assigned_to,fee_schedule_id,professional_fee,fee_status,billing_ledger_entry_id,status,notes,version").in("encounter_id",encounterIds).order("assigned_from",{ascending:false}):Promise.resolve({data:[]}),
