...
export default async function ClinicalRegistryPage(){
 ...
 const[{data:catalog},{data:encounters},{data:assignments},{data:doctorRows},{data:fees},{data:census}]=await Promise.all([
  ...
  supabase.from("encounter_care_team").select("id,encounter_id,doctor_id,role,is_primary,assigned_from,assigned_to,fee_schedule_id,professional_fee,fee_status,billing_ledger_entry_id,status,notes,version").order("assigned_from",{ascending:false}).limit(1000),
