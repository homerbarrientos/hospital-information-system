...
export default async function Patients() {
  ...
  const [{ data: assignments }, { data: patients, error }, { data: options }] = await Promise.all([
    ...
    supabase.from("reference_options").select("code,label,sort_order,reference_groups!inner(code)").in("reference_groups.code", ["sex_at_birth", "civil_status", "blood_type", "nationality", "religion", "occupation", "philhealth_membership_type", "philhealth_relationship", "philhealth_status", "government_id_type", "emergency_contact_relationship"]).eq("active", true).order("sort_order"),
  ]);
